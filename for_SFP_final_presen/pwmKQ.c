        ...
        *trainspeed = 3.3 * *speed;
        ...
        if (*isvvvf == '1' && (*notch == 'P' || (*notch != 'P' && *speed > MINSPEED))) {  // [★変更]Pノッチのときは速度によらずpwmを行う
            ...
            *Vs = *fs/180 + 0.1;
            ...
            /* パルスモード判定 */
            if (*fs > 58.0) {*pulsemode = 1;}
            else if (*fs > 52.0) {*pulsemode = 3;}
            else if (*fs > 44.0) {*pulsemode = 6;}
            else if (*fs > 36.0) {*pulsemode = 9;}
            else if (*fs > 30.5) {*pulsemode = 12;}
            else if (*fs > 26.6)  {*pulsemode = 15;}
            else if (*fs > 24.1)  {*pulsemode = 18;}
            else if (*fs > 5.00)  {*pulsemode = 0;}
            else if (*fs > 4.50)  {*pulsemode = -1;}
            else if (*fs > 4.00)  {*pulsemode = -2;}
            else if (*fs > 3.50)  {*pulsemode = -3;}
            else if (*fs > 3.00)  {*pulsemode = -4;}
            else if (*fs > 2.50)  {*pulsemode = -5;}
            else if (*fs > 2.00)  {*pulsemode = -6;}
            else if (*fs > 1.50)  {*pulsemode = -7;}
            else {*pulsemode = -8;}
