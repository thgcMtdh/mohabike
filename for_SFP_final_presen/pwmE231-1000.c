...
int main(int argc, char** argv) {
    ...
    *pulsemode = 0;
    ...
    /* PWMの計算に使う変数 */
    const double VFRATE = 0.7165;  // 電車の速度÷モータ周波数
        ...
        *trainspeed = 3.0 * *speed;
        ...
        if (*isvvvf == '1' && (*notch == 'P' || (*notch != 'P' && *speed > MINSPEED))) {  // [★変更]Pノッチのときは速度によらずpwmを行う
            ...
            *fs = *trainspeed / VFRATE;  // モータ周波数を計算
            ...
            /*  非同期モード */
            if (*pulsemode <= 0) {
                if (*pulsemode == 0) {*fc = 1050.0;}  // キャリア周波数を計算
                else if (*pulsemode == -1) {*fc = 1050 - 350 * (*fs-23)/25;}
                else if (*pulsemode == -2) {*fc = 700 + 1100 * (*fs-48)/11;}
                *frand = 0.0;  // ランダム変調幅
